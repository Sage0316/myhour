...
// 사진·짤·음성 원본의 표시용 src.
// 신규 기록은 IndexedDB(mediaId)에서 불러오고, 구버전 기록은 content의 data URL을 그대로 쓴다.
...
export function useMediaSrc(record: MyRecord): string | null {
  ...
  useEffect(() => {
    ...
    if (!record.mediaId) { setSrc(null); return; }

    let cancelled = false;
    let objectUrl: string | null = null;
    loadVideoFromIDB(record.mediaId).then(url => {
      if (cancelled) {
  ...
  }, [record.mediaId, record.content, isInlineDataUrl]);
