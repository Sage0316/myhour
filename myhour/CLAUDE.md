...
- 원본 정리 정책: **영상 없이 마감한 항목만** 3일(`TRIM_AFTER_DAYS`) 뒤 원본(사진/음성/클립)을 정리한다(`sweepArchive`). 글 텍스트·시간·캡션은 남는다. 영상까지 만든 항목(`isWrapped`)은 건드리지 않는다
- **`sweepArchive`는 아카이브 화면을 열 때만 돈다.** 백그라운드 작업이 없으므로 "정확히 3일째"가 아니라 "3일이 지난 뒤 처음 앱을 열 때" 지워진다. 이건 버그가 아니라 로컬 우선 구조의 결과다
- **정리 예고는 인앱 배지가 유일한 수단이다** (2026-08-04 사용자 결정). 푸시는 쓰지 않는다 — 워커는 사용자의 아카이브 상태를 모르고, 알게 하려면 로컬 우선 원칙을 깨야 한다. 앱을 열지 않은 사용자가 원본을 잃는 것은 감수하기로 한 비용이다
- **2026-08-04 이전엔 `sweepArchive`와 `stripRecordMedia`가 빈 함수였다** — 문서엔 3일 정리가 적혀 있는데 코드는 아무것도 지우지 않았다. 문서만 읽고 동작을 단정하지 말 것
## 핵심 원칙
...
## 감정 강도는 무드를 바꾸지 않는다 (2026-08-05)
**강도는 "그 감정의 세기"고, "다른 감정"이 아니다.** 예전엔 프롬프트가 "제목·closing·**mood**·bgmTrack이
이 강도를 반영해야 한다"고 지시해서, 강도를 올리면 모델이 잔잔함을 분노로 격상시켰다. 사용자가 보고 있던
무드가 말없이 바뀌는 건 명백한 버그다. 세 겹으로 막는다 — 하나만 있으면 뚫린다:
1. **프롬프트**: 강도가 바꾸는 것은 문체 온도와 bgmTrack의 무게뿐, `mood`/`moodChip`은 기록 내용만으로 정한다고 명시
2. **서버 강제**: 요청에 `mood`가 있으면 `normalizeResult`가 모델 응답을 무시하고 그 값으로 덮어쓴다.
   프롬프트로 부탁만 하면 모델이 어긴다 (`worker-auth-test.mjs`의 MOOD LOCK 테스트가 지킨다)
3. **클라이언트**: `applyDirector`가 무드 칩을 자동 선택하는 건 **첫 결과일 때뿐**이다
   (`firstDirectorAppliedRef`). 두 번째부터는 화면의 무드를 `moodLock`으로 같이 보낸다
- **`moodLock`은 캐시 키에 들어간다.** 고정해서 부른 결과와 고정 없이 부른 결과는 다른 응답이라
  같은 칸에 담기면 안 된다 (`directorKeyFor`의 recordsHash에 무드를 섞는다)
- 첫 호출은 `moodLock`이 `undefined`다 — AI가 기록만 보고 무드를 정해야 하기 때문이다
...
- **관리자 모드 (2026-08-04 도입, 2026-09-22 재작성)**: 설정 맨 아래 빌드 버전을 **일곱 번 두드리면** 켜진다.
  켜짐 상태는 `src/devMode.ts`(localStorage `hakku_dev_mode_v1`)에 있고 `useDevMode()`로 구독한다.
  **처음엔 `SettingsScreen`의 `useState`에만 있었다** — 탭을 옮기거나 모달을 열면 그 화면이 언마운트되면서
  (`App.tsx`의 `renderMain()`은 탭마다 다른 컴포넌트를 돌려준다) 관리자 모드가 그대로 꺼졌다.
  화면 지역 상태에 이런 걸 두지 말 것.
- **관리자 모드가 켜져 있는 동안은 "하루 한 편" 이용권과 마감 잠금을 통째로 건너뛴다.** 게이트가 네 곳이라
  하나만 고치면 여전히 막힌다: ① `context.tsx`의 `isDayWrapped`(기록 추가 잠금),
  ② `services/wrap-up-service.ts`의 `complete()`(이미 만든 날짜 거절),
  ③ `WrapUpScreen`의 `alreadyGenerated`(오늘 마감/아카이브 항목 양쪽), ④ `ArchiveScreen`의
  "영상 다시 만들기 · DEV" 버튼(영상이 이미 있는 항목엔 원래 버튼 자체가 없다).
  `wrapLock.test.ts`가 ①과 저장 유지를 지킨다
- 설정의 "오늘 마감 기록 지우기"(`unlockToday()`)는 `isWrapped` 플래그와 그 날짜의 영상 이용권을 **둘 다** 푼다 —
  하나만 풀면 여전히 잠겨 있다(`isDayWrapped`가 OR이라서). 관리자 모드를 끈 뒤에도 그 하루를 열어두고
  싶을 때 쓴다. 이미 만든 영상과 아카이브 항목은 건드리지 않는다.
  **실제 과금을 붙일 때 `releaseVideoForDate`와 `devMode`의 우회는 서버 판정으로 대체되거나 사라져야 한다**
