...
- **테스트 도구 (2026-08-04)**: 설정 맨 아래 빌드 버전을 **일곱 번 두드리면** "테스트 도구" 구역이 열리고
  "오늘 마감 잠금 풀기"가 나온다. `unlockToday()`가 `isWrapped` 플래그와 그 날짜의 영상 이용권을 **둘 다** 푼다 —
  하나만 풀면 여전히 잠겨 있다(`isDayWrapped`가 OR이라서). 잠금을 넣은 뒤 개발자도 같이 막혀서
  다음 날까지 테스트를 못 하게 된 문제 때문에 넣었다. 이미 만든 영상과 아카이브 항목은 건드리지 않는다.
  **실제 과금을 붙일 때 `releaseVideoForDate`는 서버 판정으로 대체되거나 사라져야 한다**
